...
CREATE  procedure [dbo].[usp_CurrentStatus]      
...
(      
 ...
 grn_gen_date, Invoice_Paid_date, gid, CREATED_DATE, Central_Approved_date  ,Region, Wo_Uploaded_Date  , Invoice_Status_Type   
)      
select       
 a.css_id,       
 ...
 count(a.id) as WO_COUNT, coalesce(b.INV_AMT,0),coalesce(b.Tax_amt,0), coalesce(b.inc_tax_amt,0),      
 ...
 min(b.inv_paid_Date) as Invoice_Paid_Date,''' +  @gid + ''', GETDATE(), min(a.Central_UpdatedDate)      
  ,a.Region, a.Loaded_Date as Wo_Uploaded_Date    ,b.Status_Type   as Invoice_Status_Type
from       
 se_work_order a      
...
set @sql = @sql + ' group by a.css_id,       
 ...
 b.WO_COUNT, b.INV_AMT  , b.tax_amt, b.inc_tax_amt  ,a.Region,a.Loaded_Date ,b.Status_Type    
union      
select       
 a.css_id,       
 css.css_code, css.css_Name_in_BFS_To_be_Referred as Css_Name, a.WO_BusinessUnit,      
...
from       
 ...
 left outer join (Select css_id, month_name, id as inv_id from se_css_invoice where month_name IN (' + @monthName + ')) d      
