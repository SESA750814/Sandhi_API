...
CREATE     PROCEDURE [dbo].[SP_Cooling_Rate_Expense_Mapping]
...
INNER JOIN
    Cooling_Expense WO
ON 
    ProCateList.Product like  SUBSTRING(WO.Product, 1, 3) 
	or  ProCateList.Product like  SUBSTRING(WO.Product, 1, 4)
...
--    ProCateList.Product like '%'+ SUBSTRING(WO.Product, 1, 3) + '%'
--	or  ProCateList.Product like '%'+ SUBSTRING(WO.Product, 1, 4) + '%'
--	and WO.Product_Grouping is null
...
-- Labour Total Cost Fix begin
  SELECT Parent.Work_Order_Number as Work_Order_Number, SUM( Child.Labour_Charges ) AS Total
  into #total_Labour_Cost  FROM Cooling_Work_Order AS Parent 
       LEFT JOIN Cooling_Expense As Child ON Parent.Work_Order_Number = Child.Work_Order_Number
GROUP BY Parent.Work_Order_Number;


update Cooling_Work_Order 
set Cooling_Labour_Cost = B.Total
from #total_Labour_Cost B
where
Cooling_Work_Order.Work_Order_Number like B.Work_Order_Number

Delete #total_Labour_Cost

-- Labour Total Cost Fix End
...
update Cooling_Expense
set Total_Claim = Cooling_Work_Order.Cooling_Total_Cost
from Cooling_Work_Order 
where Cooling_Expense.Work_Order_Number like Cooling_Work_Order.Work_Order_Number
