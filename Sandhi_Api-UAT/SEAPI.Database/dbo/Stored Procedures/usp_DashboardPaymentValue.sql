CREATE procedure		[dbo].[usp_DashboardPaymentValue]
...
AS
begin
-- exec usp_DashboardPaymentValue 'Region'
...
(
	...
	Cooling_Month1		decimal(18,2)	null,
	Cooling_Month2		decimal(18,2)				null,
	Cooling_Month3		decimal(18,2)				null,
	HBN_Month1			decimal(18,2)				null,
	HBN_Month2			decimal(18,2)				null,
	HBN_Month3			decimal(18,2)				null,
	PPI_Month1			decimal(18,2)				null,
	PPI_Month2			decimal(18,2)				null,
	PPI_Month3			decimal(18,2)				null
)
...
BEGIN
	...
	BEGIN
		...
		begin
			...
			while (@ordinal<7)
			begin		
				declare @invValue				decimal(18,2)=0
				if(@ordinal=1)
				begin
					select
						 @invValue= count(distinct css_id)
					From
						Se_Css_invoice a
				...
				set @sql = 'Update #tmpPayment set ' + @businessUnit + '_Month' + convert(Varchar(20),(@i+1)) + '=' + convert(Varchar(20),@invValue) + ' where ordinal=' + Convert(Varchar(10),@ordinal) +' and region=''' + @region + ''''
				exec (@sql)
...
select 
	...
	coalesce(Month1.Month1Amount,0) as Month1Amount, coalesce(Month2.Month2Amount,0) as Month2Amount, Coalesce(Month3.Month3Amount,0) as Month3Amount 
from 
	(
		Select 
	...
	(		
		select
			css_id, WO_BUSINESSUNIT, sum(INC_TAX_AMT) as Month1Amount
		From
			Se_Css_invoice 
		where
	...
	(		
		select
			css_id, WO_BUSINESSUNIT, sum(INC_TAX_AMT) as Month2Amount
		From
			Se_Css_invoice 
		where
	...
	(		
		select
			css_id, WO_BUSINESSUNIT, sum(INC_TAX_AMT) as Month3Amount
		From
			Se_Css_invoice 
		where
