...
CREATE procedure [dbo].[SP_HBN_Insert_Transaction]  
...
--,@W_Year = year(format(try_convert(datetime,Completed_On),'dd/MM/yyyy '))   from HBN_RAW_DUMP   
...
--select datename(month,try_convert(datetime,Completed_On))+'-'+ datename(year,try_convert(datetime,Completed_On))  
  
--from HBN_RAW_DUMP  
  
--group by datename(month,try_convert(datetime,Completed_On))+'-'+ datename(year,try_convert(datetime,Completed_On))  
  
--select datename(month,format(try_convert(datetime,Completed_On),'dd/MM/yyyy ')) from HBN_RAW_DUMP  
  
--select month(format(try_convert(datetime,Completed_On),'dd/MM/yyyy ')) from HBN_RAW_DUMP  
  
  
  
INSERT INTO [dbo].[HBN_Work_Order]   
           ([Work_Order_Number]  
           ,[Case]  
     ...
     ,Services_Business_Unit  
     from [dbo].[HBN_RAW_DUMP]  Where Is_Billable like 'NO'  
